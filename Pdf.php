...
$sql = "SELECT codigo, titulo, autor, editorial, categoria, idioma, formato FROM libros";
...
// Encabezados y anchos más compactos
$headers = ['Codigo', 'Titulo', 'Autor', 'Editorial', 'Categoria', 'Idioma', 'Formato'];
$anchoColumnas = [15, 40, 30, 30, 20, 20, 15]; // reducidos
...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        $pdf->Cell($anchoColumnas[0],10,$row['codigo'],1,0,'C',true);
        $pdf->Cell($anchoColumnas[1],10,utf8_decode($row['titulo']),1,0,'L',true);
        $pdf->Cell($anchoColumnas[2],10,utf8_decode($row['autor']),1,0,'L',true);
        $pdf->Cell($anchoColumnas[3],10,utf8_decode($row['editorial']),1,0,'L',true);
        $pdf->Cell($anchoColumnas[4],10,utf8_decode($row['categoria']),1,0,'C',true);
        $pdf->Cell($anchoColumnas[5],10,utf8_decode($row['idioma']),1,0,'C',true);
        $pdf->Cell($anchoColumnas[6],10,utf8_decode($row['formato']),1,0,'C',true);
...
$pdf->Output('I', 'ListadoLibros.pdf');
?>
